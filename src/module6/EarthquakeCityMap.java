...
public class EarthquakeCityMap extends PApplet {
    ...
    private void sortAndPrint(){
        ...
        for(int i=0; i<quakeMarkers.size(); i++){
        	...
        	if( m.containsKey(eq.get(i).getMagnitude()) ) {
        		m.put(eq.get(i).getMagnitude(), m.get(eq.get(i).getMagnitude())+1);
        	}
        	else {
        		m.put(eq.get(i).getMagnitude(), 1);
        	}
        }
        ArrayList<Float> maximum = new ArrayList<Float>();
        ...
        System.out.println(maximum.get(maximum.size() -1));
